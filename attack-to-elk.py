...
def parse_mitre_attack_techniques (mitre_attack_matrix):
    ...
    for technique in mitre_attack_techniques:
        ...
        # Skip if "kill_chain_phase" is not within the doc
        if 'kill_chain_phases' in technique:
            ...
            for kill_chain_phase in kill_chain_phases: 
                ...
                if data_sources is not None:
                    data_source_index.add_bulk(data_sources)

                if platforms is not None:
                    platforms_index.add_bulk(platforms)

                if groups is not None:
                    groups_index.add_bulk(groups)
                
                if software is not None:
                    software_index.add_bulk(software)
    ...
    technique_index.add_bulk(techniques)
...
def parse_mitre_attack_tactics(attack_matrix):
    ...
    return es.add_bulk(tactics)   

def main():
    util                    = utils.utils(MITRE_ATTACK_FILE_NAME, MITRE_ATTACK_URL)
    mitre_attack_matrix     = json.loads(util.read_file())['objects']
